...
frameApi.interceptors.response.use(
    (response) => response,
    async (error) => {
        ...
        if (error.response && error.response.status === 401 && !originalRequest._retry) {
            ...
            try {
                ...
                const r = await axios.post(`${BASE_API_URL}/api/authentication/token/refresh/`, {
                    refresh: refreshToken,
                });
                localStorage.setItem('access', r.data.access);
                originalRequest.headers.Authorization = `Bearer ${r.data.access}`;
                return frameApi(originalRequest);
            } catch (e) {
...
const Chat = () => {
    ...
    const speakMessage = useCallback((message) => {
        ...
        .then(async response => {
            if (!response.ok) {
                ...
                console.error('Groq TTS error body:', body);
    ...
    const startRecording = useCallback(() => {
        ...
        navigator.mediaDevices.getUserMedia({ audio: true })
            .then(stream => {
                ...
                setIsRecording(true);
                setRecordingStatus('Recording... Click "Stop Recording" when done');
            })
            .catch(error => {
    ...
    const stopRecordingAndSend = useCallback(async () => {
        ...
        // Stop silence detector immediately so it doesn't fire twice
        if (silenceIntervalRef.current) {
            clearInterval(silenceIntervalRef.current);
            silenceIntervalRef.current = null;
        }
        ...
        try {
            ...
            if (chatResponse.data.intent === 'Quit_interview' || chatResponse.data.interview_ended) {
                ...
                // Stash report for ThankYou page
                if (chatResponse.data.report) {
                    try { sessionStorage.setItem('interviewReport', JSON.stringify(chatResponse.data.report)); } catch(_) {}
                }
    ...
    // Keep latest stopRecordingAndSend in a ref so silence detector can call it
    useEffect(() => {
        stopRecordingFnRef.current = stopRecordingAndSend;
